...
// Ported from Le-Space/simple-todo apps/escrow01 (src/lib/technical-view.js) at f0d3df4.
// Changed: the storage key's prefix, and the wording below.

/**
 * Whether the page shows its technical explanations.
 *
 * One switch for the whole page. The header has it, and the consent dialog has
 * the same button: whoever wants the detail turns it on once, and every plain
 * explanation then carries its technical one next to it, including after a
 * reload. A preference flag in localStorage, nothing else.
 */
export const TECHNICAL_VIEW_STORAGE_KEY = 'belege.technicalView';
