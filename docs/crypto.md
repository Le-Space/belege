...
5. `eth_getBalance` and `alchemy_getTokenBalances` for the listed contracts only.
...
**Checked** (2026-09-26): the five endpoint hosts, and the parameters, categories, the `internal` networks, `uniqueId` shapes, the 1000 page limit, the batch limit and the error shapes (401 `-32600` _Must be authenticated!_, 403, 429 code 429, `-32601` _Unsupported method_, `-32602` _invalid … argument_) against Alchemy's documentation; that every host answers, and that a made-up key gets exactly that 401 body. **Not checked**: a real call of each method – Alchemy's public `demo` key answered only HTTP 429 (empty body) from here all day – so the request formats rest on the documentation and on the fake (`bridge/test/support/fake-alchemy.js`, as strict as the documented API about method names, parameter count and types, e.g. a string `"true"` where a boolean belongs is refused); the trace-address format of internal `uniqueId`s (the bridge takes whatever follows `:internal:`, letters, digits, `_ , -`).
