...
5. `eth_getBalance` und `alchemy_getTokenBalances` nur für die gelisteten Verträge.
...
**Geprüft** (26.09.2026): die fünf Endpunkt-Hosts sowie Parameter, Kategorien, die Netze mit `internal`, die Form der `uniqueId`, die Seitengröße 1000, die Batch-Grenze und die Fehlerformen (401 `-32600` _Must be authenticated!_, 403, 429 mit Code 429, `-32601` _Unsupported method_, `-32602` _invalid … argument_) an Alchemys Dokumentation; dass jeder Host antwortet und ein ausgedachter Schlüssel genau diese 401-Antwort bekommt. **Nicht geprüft**: ein echter Aufruf jeder Methode – Alchemys öffentlicher `demo`-Schlüssel antwortete von hier den ganzen Tag nur HTTP 429 (leerer Rumpf) –, die Anfrageformen stützen sich also auf die Dokumentation und auf das Fake (`bridge/test/support/fake-alchemy.js`, so streng wie die dokumentierte API bei Methodennamen, Anzahl und Typen der Parameter, z. B. wird ein String `"true"` statt eines Booleans abgelehnt); die Form der Trace-Adresse in internen `uniqueId`s (die Bridge nimmt, was nach `:internal:` folgt: Buchstaben, Ziffern, `_ , -`).
