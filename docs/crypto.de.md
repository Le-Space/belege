...
- **EVM**: die Etherscan-kompatible API von Blockscout, ohne Schlüssel: `txlist`, `txlistinternal`, `tokentx`, `balance`, `tokenbalance`. Über das 10 000-Einträge-Fenster von Blockscout hinaus wird nach Startblock weitergeblättert, bei dessen Ratenlimit wiederholt.
...
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| gesendet / empfangen                                       | movement `transfer`, sourceId `<hash>:<n>`, `counterpartyAddress` = die andere Adresse | nötig – außer die andere Adresse ist eine eigene Wallet          |
...
| Delegation (an `bonded_tokens_pool`)                       | movement `stake`                                                                       | keiner: die Tokens bleiben unsere; Konto klärt der Steuerberater |
| IBC-Transfer hinaus                                        | movement `transfer`, der Empfänger auf der anderen Chain als Gegenpartei               | nötig                                                            |

`txRef` ist der Hash genau so, wie die Chain ihn angibt: bei Cosmos Hex in Großbuchstaben, bei EVM `0x` + Hex in Kleinbuchstaben. Eine Börse, die den On-Chain-Hash einer Auszahlung oder Einzahlung nennt, lässt sich darüber paaren (die Kraken-Seite: #52). Jede Buchung behält `explorerUrl`, ihre Transaktion im Block-Explorer; _Zahlungen_ zeigt _Im Block-Explorer ansehen_ und die _Gegenadresse_.
